...
def test_on_env(max_steps, ctrl, df, x0, z0, iter, normalize):
    ...
    try:
        ...
        plt.close("all")
        plt.figure(0)
        plt.plot(local_t, env.Y, label='Real Output')
        plt.plot(local_t, df.E, label='EMPC Output')
        plt.plot(local_t, df.ESS, linestyle="--", label='Steady State Output')
        plt.xlabel('Time [min]')
        ...
        axs[0, 0].plot(local_t, df.U1 * 1000, label='EMPC Output')
        axs[0, 0].plot(local_t, df.USS1 * 1000, linestyle="--", label='Steady State Input')
        axs[0, 0].set_ylabel(u'U1 [\u03bcL/min]')
        axs[0, 0].set_xlabel('time [min]')
        axs[0, 0].grid()

        axs[0, 1].step(local_t, U[:, 1], where='post', label='Real Input')
        axs[0, 1].plot(local_t, df.U2 * 1000, label='EMPC Output')
        axs[0, 1].plot(local_t, df.USS2 * 1000, linestyle="--", label='Steady State Input')
        axs[0, 1].set_ylabel(u'U2 [\u03bcL/min]')
        axs[0, 1].set_xlabel('time [min]')
        axs[0, 1].grid()

        axs[1, 0].step(local_t, U[:, 2], where='post', label='Real Input')
        axs[1, 0].plot(local_t, df.U3 * 1000, label='EMPC Output')
        axs[1, 0].plot(local_t, df.USS3 * 1000, linestyle="--", label='Steady State Input')
        axs[1, 0].set_ylabel(u'U3 [\u03bcL/min]')
        axs[1, 0].set_xlabel('time [min]')
        axs[1, 0].grid()

        axs[1, 1].step(local_t, U[:, 3], where='post', label='Real Input')
        axs[1, 1].plot(local_t, df.U4 * 1000, label='EMPC Output')
        axs[1, 1].plot(local_t, df.USS4 * 1000, linestyle="--", label='Steady State Input')
